...
"Line 6 we define a function that calculates the winner of the election based on the elector numbers in dictionary"
" and hypthetical"

"then in for loop at line 13 goes through elctoral dictionary calculates the number of voters according to each party"

"winner checks which one has the most voters r or d"

" other for loop line 27, finds out which one is a winner whether r or d"

def get_winner(electoral, hypothetical):

    results={}

    for key in electoral:

        e=electoral[key]

        h=hypothetical[key]

        results[h]=results.get(h,0)+e

    winner=h

    for key in results:

        if results[key] > results[h]:
